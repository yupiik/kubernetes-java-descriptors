package io.yupiik.kubernetes.bindings.v1_23_0;

import java.util.Objects;

public class CertificateSigningRequestListItemsStatusConditions {
    private String lastTransitionTime;
    private String lastUpdateTime;
    private String message;
    private String reason;
    private String status;
    private CertificateSigningRequestListItemsStatusConditionsType type;

    public CertificateSigningRequestListItemsStatusConditions() {
        // no-op
    }

    public CertificateSigningRequestListItemsStatusConditions(final String lastTransitionTime,
                                                              final String lastUpdateTime,
                                                              final String message,
                                                              final String reason,
                                                              final String status,
                                                              final CertificateSigningRequestListItemsStatusConditionsType type) {
        // no-op
    }

    public String getLastTransitionTime() {
        return lastTransitionTime;
    }

    public void setLastTransitionTime(final String lastTransitionTime) {
        this.lastTransitionTime = lastTransitionTime;
    }

    public String getLastUpdateTime() {
        return lastUpdateTime;
    }

    public void setLastUpdateTime(final String lastUpdateTime) {
        this.lastUpdateTime = lastUpdateTime;
    }

    public String getMessage() {
        return message;
    }

    public void setMessage(final String message) {
        this.message = message;
    }

    public String getReason() {
        return reason;
    }

    public void setReason(final String reason) {
        this.reason = reason;
    }

    public String getStatus() {
        return status;
    }

    public void setStatus(final String status) {
        this.status = status;
    }

    public CertificateSigningRequestListItemsStatusConditionsType getType() {
        return type;
    }

    public void setType(final CertificateSigningRequestListItemsStatusConditionsType type) {
        this.type = type;
    }

    @Override
    public int hashCode() {
        return Objects.hash(
                lastTransitionTime,
                lastUpdateTime,
                message,
                reason,
                status,
                type);
    }

    @Override
    public boolean equals(final Object __other) {
        if (!(__other instanceof CertificateSigningRequestListItemsStatusConditions)) {
            return false;
        }
        final CertificateSigningRequestListItemsStatusConditions __otherCasted = (CertificateSigningRequestListItemsStatusConditions) __other;
        return Objects.equals(lastTransitionTime, __otherCasted.lastTransitionTime) &&
            Objects.equals(lastUpdateTime, __otherCasted.lastUpdateTime) &&
            Objects.equals(message, __otherCasted.message) &&
            Objects.equals(reason, __otherCasted.reason) &&
            Objects.equals(status, __otherCasted.status) &&
            Objects.equals(type, __otherCasted.type);
    }
}
